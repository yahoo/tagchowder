 ...
 *
 * <p>
 * This class can be used by itself or as part of a SAX event stream: it takes as input a series of SAX2 ContentHandler events and uses the
 * information in those events to write an XML document. Since this class is a filter, it can also pass the events on down a filter chain for further
 * processing (you can use the XMLWriter to take a snapshot of the current state at any point in a filter chain), and it can be used directly as a
 * ContentHandler for a SAX2 XMLReader.
 * </p>
 *
 * <p>
 * The client creates a document by invoking the methods for standard SAX2 events, always beginning with the {@link #startDocument startDocument}
 * method and ending with the {@link #endDocument endDocument} method. There are convenience methods provided so that clients to not have to create
 * empty attribute lists or provide empty strings as parameters; for example, the method invocation
 * </p>
 *
 * <pre>
 * w.startElement("foo");
 ...
 *
 * <p>
 * Except that it is more efficient because it does not allocate a new empty attribute list each time. The following code will send a simple XML
 ...
 *
 * <p>
 * In fact, there is an even simpler convenience method, <var>dataElement</var>, designed for writing elements that contain only character data, so
 ...
 *
 * <p>
 * According to the XML Recommendation, <em>all</em> whitespace in an XML document is potentially significant to an application, so this class never
 ...
 *
 * <p>
 * The writer contains extensive support for XML Namespaces, so that a client application does not have to keep track of prefixes and supply
 * <var>xmlns</var> attributes. By default, the XML writer will generate Namespace declarations in the form _NS1, _NS2, etc., wherever they are
 ...
 *
 * <p>
 * In many cases, document authors will prefer to choose their own prefixes rather than using the (ugly) default names. The XML writer allows two
 * methods for selecting prefixes:
 * </p>
 *
 * <ol>
 ...
 *
 * <p>
 * Whenever the XML writer finds a new Namespace URI, it checks to see if a qualified (prefixed) name is also available; if so it attempts to use the
 * name's prefix (as long as the prefix is not already in use for another Namespace URI).
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * By default, the XML writer will not declare a Namespace until it is actually used. Sometimes, this approach will create a large number of Namespace
 ...
 *
 * <p>
 * The "rdf" prefix is declared only once, because the RDF Namespace is used by the root element and can be inherited by all of its descendants; the
 * "dc" prefix, on the other hand, is declared three times, because no higher element uses the Namespace. To solve this problem, you can instruct the
 * XML writer to predeclare Namespaces on the root element even if they are not used there:
 * </p>
 *
 * <pre>
...
public class XMLWriter extends XMLFilterImpl implements LexicalHandler {
     ...
     * It is usually a good idea to invoke {@link #flush flush} before resetting the writer, to make sure that no output is lost.
     * </p>
     *
     * <p>
     ...
     *
     * <p>
     * <strong>Note:</strong> this method will <em>not</em> clear the prefix or URI information in the writer or the selected output writer.
     * </p>
     *
     * @see #flush
     */
     ...
     *
     * <p>
     * This method flushes the output stream. It is especially useful when you need to make certain that the entire document has been written to
     * output but do not want to close the output stream.
     * </p>
     *
     * <p>
     ...
     *
     * <p>
     * Note that this method does not actually force the Namespace to be declared; to do that, use the {@link #forceNSDecl(java.lang.String)
     ...
     *
     * <p>
     * By default, the XMLWriter will declare only the Namespaces needed for an element; as a result, a Namespace may be declared many places in a
     * document if it is not used on the root element.
     * </p>
     *
     * <p>
     * This method forces a Namespace to be declared on the root element even if it is not used there, and reduces the number of xmlns attributes in
     * the document.
     * </p>
     *
     ...
     *
     * @exception org.xml.sax.SAXException If there is an error writing the XML declaration, or if a handler further down the filter chain raises an
     *                exception.
     ...
     * Pass the event on down the filter chain for further processing.
     *
     * @exception org.xml.sax.SAXException If there is an error writing the newline, or if a handler further down the filter chain raises an
     *                exception.
     ...
     * @param qName The element's qualified (prefixed) name, or the empty string is none is available. This method will use the qName as a template
     *            for generating a prefix if necessary, but it is not guaranteed to use the same qName.
     * @param atts The element's attribute list (must not be null).
     * @exception org.xml.sax.SAXException If there is an error writing the start tag, or if a handler further down the filter chain raises an
     *                exception.
     ...
     * @param qName The element's qualified (prefixed) name, or the empty string is none is available. This method will use the qName as a template
     *            for generating a prefix if necessary, but it is not guaranteed to use the same qName.
     * @exception org.xml.sax.SAXException If there is an error writing the end tag, or if a handler further down the filter chain raises an
     *                exception.
     * @see org.xml.sax.ContentHandler#endElement
     */
    @Override
    public void endElement(final String uri, final String localName, final String qName) throws SAXException {
        if (!(htmlMode && (uri.equals("http://www.w3.org/1999/xhtml") || uri.equals(""))
                && (qName.equals("area") || qName.equals("base") || qName.equals("basefont") || qName.equals("br") || qName.equals("col")
                        || qName.equals("frame") || qName.equals("hr") || qName.equals("img") || qName.equals("input") || qName.equals("isindex")
     ...
     * @exception org.xml.sax.SAXException If there is an error writing the whitespace, or if a handler further down the filter chain raises an
     *                exception.
     ...
     *
     * This method writes an empty element tag rather than a start tag followed by an end tag. Both a {@link #startElement startElement} and an
     * {@link #endElement endElement} event will be passed on down the filter chain.
     *
     ...
     * @param qName The element's qualified name (with prefix), or the empty string if none is available. This parameter is strictly advisory: the
     *            writer may or may not use the prefix attached.
     * @param atts The element's attribute list.
     * @exception org.xml.sax.SAXException If there is an error writing the empty tag, or if a handler further down the filter chain raises an
     *                exception.
     * @see #startElement
     * @see #endElement
     */
     ...
     *
     * <p>
     * This method will provide an empty string for the Namespace URI, and empty string for the qualified name, and a default empty attribute list. It
     ...
     *
     * <p>
     * This method invokes {@link #startElement(String, String, String, Attributes)}, followed by {@link #characters(String)}, followed by
     ...
     * @see #endElement(String, String, String)
     */
    public void dataElement(final String uri, final String localName, final String qName, final Attributes atts, final String content)
            throws SAXException {
     ...
     *
     * <p>
     * This is a convenience method to write a complete element with character data content, including the start tag and end tag. This method provides
     ...
     *
     * <p>
     * This is a convenience method to write a complete element with character data content, including the start tag and end tag. The method provides
     * an empty string for the Namespace URI, and empty string for the qualified name, and an empty attribute list.
     * </p>
     *
     * <p>
     * This method invokes {@link #startElement(String, String, String, Attributes)}, followed by {@link #characters(String)}, followed by
     ...
     * @exception org.xml.sax.SAXException If there is an error writing the character, this method will throw an IOException wrapped in a
     *                SAXException.
     */
     ...
     * @exception org.xml.SAXException If there is an error writing the attribute list, this method will throw an IOException wrapped in a
     *                SAXException.
     */
    ...
    private String[] booleans = { "checked", "compact", "declare", "defer", "disabled", "ismap", "multiple", "nohref", "noresize", "noshade",
     ...
     *
     * @exception org.xml.sax.SAXException This method will throw an IOException wrapped in a SAXException if there is an error writing the Namespace
     *                declarations.
     */
    ...
    public void startDTD(final String name, final String publicid, final String systemid) throws SAXException {
        ...
        char sysquote = (sid.indexOf('"') != -1) ? '\'' : '"';
        if (overridePublic != null) {
            pid = overridePublic;
        }
        if (!(pid == null || "".equals(pid))) {
            char pubquote = (pid.indexOf('"') != -1) ? '\'' : '"';
            write(" PUBLIC ");
            write(pubquote);
            write(pid);
            write(pubquote);
